    ...
    internal class TextRender : ContextDependent
    {
        ...
        public static void LoadData()
        {
            try
            {
                if (_hzkBuf == null)
                {
                    string _hzkBufPath = Application.streamingAssetsPath + "/Font/HZK16.bin";
                    UnitySimulator.Instance.StartCoroutine(Tool.LoadData(_hzkBufPath, delegate (byte[] data)
                    {
                        if (data != null)
                        {
                            _hzkBuf = data;
                ...
                if (_ascBuf == null)
                {
                    string _ascBufPath = Application.streamingAssetsPath + "/Font/ASC16.bin";
                    UnitySimulator.Instance.StartCoroutine(Tool.LoadData(_ascBufPath, delegate (byte[] data)
                    {
                        if (data != null)
                        {
                            _ascBuf = data;
